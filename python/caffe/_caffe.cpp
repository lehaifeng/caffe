...
struct NdarrayConverterGenerator::apply<Dtype*> {
  struct type {
    PyObject* operator() (Dtype* data) const {
      // Just store the dato pointer, and add the shape information in postcall.
...
BOOST_PYTHON_MODULE(_caffe) {
  ...
  // See GetPythonLayer for why we use auto_ptr here.
  bp::class_<Layer<Dtype>, std::auto_ptr<PythonLayer<Dtype> >,
